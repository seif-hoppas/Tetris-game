...
def is_game_over_condition(board, falling_piece):
    """Check if the game has reached a "Game Over" condition."""
    # Check if the falling piece cannot be placed at its current position
...
def count_columns_with_holes(board):
    """Count the number of columns with at least one hole."""
    holes_count = 0

    for col in range(BOARDWIDTH):
        # Check if the column has at least one hole
        if any(board[col][row] == BLANK for row in range(BOARDHEIGHT)):
            holes_count += 1

    return holes_count

def calculate_deepest_well(board):
    """Calculate the depth of the deepest well on the board."""
    deepest_well_depth = 0

    for col in range(BOARDWIDTH):
        # Initialize variables for tracking the well depth in the current column
        well_depth = 0
        current_height = 0
        ...
        for row in range(BOARDHEIGHT):
            ...
            if board[col][row] == BLANK:
                # Increment the depth of the current well
                well_depth += 1
            else:
                # Update the maximum depth of the deepest well found so far
                deepest_well_depth = max(deepest_well_depth, well_depth)
                # Reset the well depth counter
                well_depth = 0
        ...
        # Handle the case where the well reaches the bottom of the column
        if well_depth > 0:
            deepest_well_depth = max(deepest_well_depth, well_depth)

    return deepest_well_depth
...
def draw_status(score, level,moves):
    """Draw status"""
    ...
    moveSurf = BASICFONT.render('Moves: %s' % moves, True, TEXTCOLOR)
    moveRect = levelSurf.get_rect()
    moveRect.topleft = (WINDOWWIDTH - 150, 140)
...
def draw_next_piece(piece):
    ...
    next_rect.topleft = (WINDOWWIDTH - 150, 170)
    DISPLAYSURF.blit(next_surf, next_rect)

    # draw the "next" piece
    draw_piece(piece, pixelx=WINDOWWIDTH-150, pixely=180)
...
def calc_move_info(board, piece, x, r, total_holes_bef, total_blocking_bloks_bef):
    ...
    for x2 in range(0, BOARDWIDTH):
        b = calc_holesAndWeights(new_board, x2)
        total_holes += b[0]
    ...
    columns_with_holes = count_columns_with_holes(board)
    deepest_well_depth = calculate_deepest_well(board)
    bumb = calc_bumpiness(board)
    ag_h = calc_aggregate_height(board)


    return [True,num_removed_lines, new_holes, new_blocking_blocks, piece_sides, floor_sides, wall_sides, columns_with_holes,bumb,ag_h]
...
def calc_initial_move_info(board):
    ...
    for x2 in range(0, BOARDWIDTH):
        b = calc_holesAndWeights(board, x2)
...
def calc_holesAndWeights(board, x):
...
def count_empty_columns(board):
    """Count the number of columns without any blocks."""
    empty_count = 0

    for col in range(BOARDWIDTH):
        # Check if the column has any blocks
        if all(board[col][row] == BLANK for row in range(BOARDHEIGHT)):
            empty_count += 1

    return empty_count

def get_column_height(board, col):
    if col < 0 or col >= BOARDWIDTH:
        return 0  # Return 0 if the column index is out of range
    for y in range(BOARDHEIGHT-1, -1, -1):
        if board[col][y] != BLANK:
...
def calc_bumpiness(board):
    ...
    for x in range(BOARDWIDTH - 1):
        height_diff = abs(get_column_height(board, x) - get_column_height(board, x + 1))
        bumpiness += height_diff
    return bumpiness

def calc_aggregate_height(board):
    total_height = 0
    for x in range(BOARDWIDTH):
        column_height = get_column_height(board, x)
